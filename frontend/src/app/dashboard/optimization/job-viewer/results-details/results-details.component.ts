        ...
        map(([opt, eqWeights]) => {
          ...
          if (opt.goal !== OptimizeGoal.EQUAL_WEIGHT) {
            comparisonSeries.push(
              ChartHelpers.portfolioReturnsToSeries(eqWeights.goal, this.portfolio.price_dates, eqWeights.portfolio_returns)
            );
          }
          comparisonSeries.push(
            ChartHelpers.portfolioReturnsToSeries(
            globalVars.GET_BENCHMARK_NAME(this.portfolio.benchmark_index.asset_data.ticker),
            this.portfolio.price_dates, this.portfolio.benchmark_index.returns
          ));
              ...
              series: [
                {
                  ...
                  data: opt.weights
                    .map((w, i) => ({
                      name: `${prefixFn(w)}${this.portfolio.parameters.tickers[i]}`,
                      y: Math.abs(w) * 100,
                      prefix: prefixFn(w)
                    }))
                    .filter(({y}) => y > 0.001)
